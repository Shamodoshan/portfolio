...
export default function HomePage() {
    ...
    const getExperience = () => {
        ...
        if (totalMonths < 12) {
            return { value: totalMonths, label: "Months Experience" };
        } else {
            const years = Math.floor(totalMonths / 12);
            const months = totalMonths % 12;
            if (months === 0) {
                return { value: years, label: `Year${years > 1 ? 's' : ''} Experience` };
            }
            return { value: years, label: `Year${years > 1 ? 's' : ''} ${months} Month${months > 1 ? 's' : ''} Exp.` };
        }
    };
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-gradient-to-br from-purple-600 to-indigo-600 p-6 rounded-2xl text-white shadow-lg">
                                <div className="text-4xl font-bold mb-1">{experience.value}</div>
